...
const Users = () => {
    ...
    const leftToolbarTemplate = () => {
        return (
            <div className="flex items-center justify-center space-x-2">
                <button className="inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle transition-all rounded-lg cursor-pointer bg-gradient-to-tl from-blue-700 to-blue-300 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs" onClick={() => handleCreateUser()} >Nouveau <AiOutlinePlus className="h-6 w-6 text-white inline"/></button>
    ...
    const actionBodyTemplate = (rowData) => {
        ...
        <button type="button" onClick={() => handleUpdateUser(rowData)} className="inline-block px-6 py-3 font-bold text-center text-white uppercase align-middle transition-all rounded-lg cursor-pointer bg-gradient-to-tl from-blue-700 to-blue-300 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs" ><BsFillPenFill className="text-white inline"/></button>
        </div>;
        
    }

    const header = renderHeader();
    const roleTemplate = (row) => row.role.join(',');
        ...
        <div className="flex flex-wrap -mx-3">
          <div className="max-w-full px-3 lg:w-1/2 lg:flex-none">
            <div className="flex items-center justify-center h-full space-x-2">
              ...
              <FaUsers className="w-20 h-20 text-green-500"/>
            </div>
          </div>
   ...
   <div className="datatable-doc mt-4 w-4/5 mx-auto">
            ...
            <Toolbar className="mb-4" left={leftToolbarTemplate}></Toolbar>
                <DataTable value={users} paginator className="p-datatable-customers" header={header} rows={10}
                    ...
                    <Column field="role" header="Role" body={roleTemplate} sortable  style={{ minWidth: '8rem' }}/>
